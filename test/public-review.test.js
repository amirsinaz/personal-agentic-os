...
import { buildReviewId, buildReviewSummary, parsePorcelain, reviewStatusFor } from "../scripts/public-review-lib.js";
...
test("keeps an approved review approved when the detected content has not changed",()=>{
  assert.equal(reviewStatusFor({id:"same",status:"approved"},"same"),"approved");
  assert.equal(reviewStatusFor({id:"older",status:"approved"},"new"),"pending");
});
