...
9. For a new Vault, create the generic structure. For an existing Vault, show a backup plan. Then transfer only verified current state, accepted decisions, open questions, and next actions for approved projects. Never store transcripts, hidden reasoning, or secrets.
10. Run the first sync and report each project as created, updated, unchanged, uncertain, or rejected. Do not write uncertain matches without approval.
11. Rebuild the dashboard from fresh state. Project count, connected tools, last-sync status, and usage data must come only from my data; missing evidence stays `Unavailable`.
