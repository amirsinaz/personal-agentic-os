...
if(result.requiredUpdate){process.stdout.write(`REQUIRED UPDATE: Personal Agentic OS ${result.latestVersion}. ${result.message} ${result.releaseUrl}\n`);process.exitCode=2;}
else process.stdout.write(result.updateAvailable?`Personal Agentic OS ${result.latestVersion} is available: ${result.releaseUrl}\n`:`Personal Agentic OS is current (${result.currentVersion}).\n`);
